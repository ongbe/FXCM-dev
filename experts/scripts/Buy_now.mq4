...
ticket= OrderSend(Symbol(),OP_BUY,1,Ask,3, 0,0, "Manuel Buy" ,magicNumber1,0,DarkGreen);
 ...
 Print("Closing script ",ticket);
  ...
  //if (Symbol() != "FRA40" || Symbol() != "GER30" || Symbol() != "UK100"   )return(0); 
  
/*
  Print( " STOPLEVEL " , MarketInfo(Symbol(),MODE_STOPLEVEL));
  Print( " MODE_TICKVALUE " , MarketInfo(Symbol(),MODE_TICKVALUE));
  Print( " MODE_TICKSIZE " , MarketInfo(Symbol(),MODE_TICKSIZE));
  Print ( getLotSize());
  Print(SlLastBar(1,Bid, 5));
*/



//   ticket= OrderSend(Symbol(),OP_BUY,1,Bid,3, 0,0, "Manuel Buy",230380,0,Green);
