...
string HtmlAdress = "http://www.dailyfx.com/files/";
string source ;

extern string GetrightAdress = "c:\getright\getright.exe";
...
{
  Print("Downloading "+StringConcatenate(HtmlAdress,source)+" to experts\files\News.csv");
  WinExec(GetrightAdress+" /URL:"+StringConcatenate(HtmlAdress,source)+" /FILE:News.csv /W /O",0);
}
...
datetime ToDate(string stDate,string stTime) 
{
  ...
  datetime Tm = StrToTime(StringConcatenate(StrToInteger(StringSubstr(stTime,0,2))+4, ":" , StringSubstr(stTime,3,2)));
  ...
  if (StringFind(stTime,"PM")>=0)       
    Res+=12*60*60;
  Res=Res-GetShiftGMT();
  //Print(stTime," - > " ,StringConcatenate(StrToInteger(StringSubstr(stTime,0,2))+4, ":" , StringSubstr(stTime,3,2)),  " ", TimeHour(Tm ), " ", TimeHour(Res));
  return (Res);
}

void GrabNews() 
{
  int file = FileOpen("\News.csv",FILE_READ|FILE_CSV,',');
  ...
  while (!FileIsEnding(file))
    {
      ...
      if (stImportance == "High" || stImportance == "Medium")
      {         
         if (stCurrency == "USD" || stCurrency == "EUR" || stCurrency == "usd" || stCurrency == "eur"  )
         {
...
//----
   if (TimeCurrent()>LastTimeDownloading+15*60)
     {
     string tmp = Day()-DayOfWeek();
     
     if (StrToInteger(tmp) < 10) tmp = StringConcatenate("0", tmp);
     
     source = StringConcatenate("Calendar-", Month(),"-",tmp,"-", Year(), ".csv");
     
     //Print(StringConcatenate(HtmlAdress,source));
     ...
     //Print(source);
       ...
       int file=-1;
       while (file==-1)
         file = FileOpen("\News.csv",FILE_READ|FILE_CSV,',');
       FileClose(file);
